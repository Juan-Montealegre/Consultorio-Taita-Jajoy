// Endpoint para cancelar una cita (admin)
...
// Endpoint para marcar una cita como completada (taita)
app.put('/api/citas/:id/completar', async (req, res) => {
  const { id } = req.params;
  try {
    await pool.query('UPDATE citas SET estado = $1 WHERE id = $2', ['completada', id]);
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
