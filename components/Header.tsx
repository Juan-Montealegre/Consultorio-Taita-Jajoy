...
const Header: React.FC<HeaderProps> = ({ onNavigate, currentUser, onLogout, isAdmin }) => {
  ...
  return (
    <>
      <header className="bg-background/80 backdrop-blur-sm text-text-dark sticky top-0 z-50 shadow-lg shadow-primary/10">
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
          ...
          >
            ...
            <span className="text-xl font-serif font-bold tracking-wider text-primary">Taita Jajoy</span>
          </div>
          ...
          <nav className="hidden md:flex items-center space-x-6 font-semibold">
            ...
            {currentUser ? (
              <>
                <button onClick={() => onNavigate(Page.MyAccount)} className="hover:text-secondary transition duration-300">{isAdmin ? 'Admin Citas' : 'Mis Citas'}</button>
          ...
          <div className="md:hidden">
            <button onClick={() => setIsMenuOpen(true)} aria-label="Abrir menú" className="text-text-dark">
      ...
      <div className={`fixed inset-0 z-[100] bg-background/95 backdrop-blur-sm transition-opacity duration-300 ease-in-out md:hidden ${isMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        <div className="flex justify-end p-6">
           <button onClick={() => setIsMenuOpen(false)} aria-label="Cerrar menú" className="text-text-dark">
             <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
               <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
             </svg>
           </button>
        </div>
        <nav className="flex flex-col items-center justify-center h-full -mt-20 text-2xl space-y-8 font-semibold text-text-dark">
          ...
          {currentUser ? (
            <>
              <button onClick={() => handleMobileNavigate(Page.MyAccount)} className="hover:text-secondary transition duration-300">{isAdmin ? 'Admin Citas' : 'Mis Citas'}</button>
